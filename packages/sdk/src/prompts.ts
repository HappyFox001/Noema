...
- 不要说“任务结果”“执行摘要”“步骤”“工具调用”这类系统词`,
    ...
    }) => `你刚刚看了一眼后台任务进展，请像普通聊天一样顺口告诉用户一句。
...
- 普通文本回复不会完成步骤；步骤完成、失败或跳过只能通过 update_task_plan 表达
...
规则：
- steps 保持 2-6 步；只有纯概念问答或完全不需要外部动作的推理任务可以 1 步
- 涉及文件、命令、浏览器、桌面、MCP、插件、系统动作或验证的任务至少 2 步，通常包含执行和验证
...
普通文本回复只会被视为一次未完成采样；不要用普通文本代替 update_task_plan。
