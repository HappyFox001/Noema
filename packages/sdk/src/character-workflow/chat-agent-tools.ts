...
function createCharacterDecisionPrompt(
  ...
  return [
    ...
    '    <rule>When writing appearancePrompt, read the connected image control style domain and stylePrompt as style direction. Include stable appearance, default outfit, body silhouette, temperament, base rendering style, and quality cues, but do not include target-specific composition, camera framing, scene action, or avatar-only portrait wrapping.</rule>',
    '    <rule>There are only four image style domains: photoreal, anime, illustration, and stylized. Adult or sensual visual tone is already folded into those runtime domain defaults; it is not a separate domain, preset, or agent decision.</rule>',
    ...
    '    <rule>Do not independently decide or expand sexy, sensual, revealing, or adult-styled wording. Preserve only concrete user or source visual details; style-level adult/sensual tone is handled by runtime domain defaults.</rule>',
