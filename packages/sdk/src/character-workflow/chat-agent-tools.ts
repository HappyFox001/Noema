...
function createCharacterDecisionPrompt(
  ...
  return [
    ...
    '    <rule>When writing appearancePrompt, read the connected image control style domain and stylePrompt as style direction. Include stable appearance, default outfit, body silhouette, temperament, base rendering style, and quality cues, but do not include target-specific composition.</rule>',
    '    <rule>There are only four image style domains: photoreal, anime, illustration, and stylized. Adult or sensual direction is not a separate domain or preset; when explicitly present in stylePrompt or the user goal, merge those cues into the selected domain language.</rule>',
    ...
    '    <rule>Avatar targetPrompts should only add the avatar composition: half-body portrait, clear face, looking at camera, calm or confident expression, simple background, soft light. Do not restate the full appearancePrompt.</rule>',
    ...
    '    <example action="request_image" target="avatar-image-target">half-body portrait, clear face, looking at camera, calm confident expression, simple pale gray background, soft natural light</example>',
    ...
    '    <field_content_rule>appearancePrompt is the avatar identity prompt: after the visible character-card fields exist, derive a compact image-model prompt from name, description, appearance, personality, background, scenario, worldContext, and connected image style controls. Include stable appearance, default outfit language, body silhouette, temperament, and a lightweight base rendering style. For anime characters, include cues like anime style illustration, expressive detailed eyes, character-card portrait, pixiv style, high quality anime art. For photoreal characters, include cues like realistic portrait photography, natural skin texture, soft natural light, shallow depth of field. Include adult/sensual cues only when stylePrompt, the user goal, or source material explicitly asks for that direction; merge them into the current photoreal/anime/illustration/stylized domain instead of creating a separate style category. Do not include story scene actions, workflow terms, reference links, multi-image layouts, panels, negative prompts, or target-specific asset instructions.</field_content_rule>',
...
function renderImageControlContextXml(control: AgentImageGenerationControl): string[] {
  return [
      `          <image_control node="${xmlEscape(control.nodeId)}" count="${control.targetImageCount}" style_domain="${xmlEscape(control.imageStyleDomain)}" style="${xmlEscape(control.stylePrompt)}" shot="${xmlEscape(control.shotType)}" aspect_ratio="${xmlEscape(control.aspectRatio)}" consistency="${xmlEscape(control.consistencyMode)}" seed="${xmlEscape(control.seedMode)}" />`,
  ]
}
