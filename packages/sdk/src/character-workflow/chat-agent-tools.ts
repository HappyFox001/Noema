...
function createCharacterDecisionPrompt(
  ...
  return [
    ...
    '    <field_content_rule>appearancePrompt must follow the Sugar-style reusable image prompt role: after the visible character-card fields exist, derive a compact image-model prompt from name, description, appearance, personality, background, scenario, and worldContext. Include stable appearance, default outfit language, and a lightweight base rendering style. For anime characters, include cues like anime style illustration, expressive detailed eyes, upper-body portrait, high quality anime art. For photoreal characters, include cues like realistic portrait photography, natural skin texture, soft natural light, shallow depth of field. If the character style is adult, sensual, seductive, NSFW, sexy, mature, streamer, glamour, or similar, include concrete Sugar-style visual cues such as alluring gaze, elegant body curves, lace, silk, sheer fabric, deep V neckline, cleavage, waistline, hip curve, warm intimate lighting, or tasteful skin exposure when they fit the character. Do not include story scene actions, workflow terms, reference links, multi-image layouts, panels, negative prompts, or target-specific asset instructions.</field_content_rule>',
    ...
    '    <image_rule>The avatar prompt must be a compact positive portrait direction for one final avatar.jpg: one visible character, one face, bust or upper body, clear eyes, simple background, polished finish.</image_rule>',
    '    <image_rule>The character-overview-sheet prompt must be a compact positive design-sheet direction: same character, wide clean canvas, front/back/side views, hairstyle, hands, feet, outfit details, expression callouts, no written labels.</image_rule>',
    ...
    '    <image_rule>The runtime will wrap every targetPrompts.prompt with a compact Sugar-style image prompt: appearancePrompt, role-specific positive visual direction, style suffix, quality suffix, and a short avoid list. Do not repeat the full appearancePrompt inside targetPrompts.prompt.</image_rule>',
    ...
    '    <image_rule>Each targetPrompts.prompt should be a compact slot-specific visual direction: pose, expression, framing, camera angle, background simplicity, prop/motif, mood, and image function. Include only identity details that are unique to this slot.</image_rule>',
...
function createSchemaXml(): string {
  return [
    ...
    '    <rule>appearancePrompt is the only hidden reusable appearance prompt for the image model. It is derived from the completed character-card fields and may include stable appearance, default outfit language, and lightweight base rendering cues such as anime style illustration or realistic portrait photography. It must not carry story scene actions, workflow terms, reference links, multi-image layouts, panels, negative prompts, or target-specific asset instructions.</rule>',
    '  </fixed_schema>',
  ].join('\n')
}
...
function supportFieldDescription(field: string): string {
  const descriptions: Record<string, string> = {
    appearancePrompt: 'Compact reusable image prompt derived from completed character-card fields; stable appearance, default outfit language, and lightweight base rendering cues are allowed, but no story scene actions, workflow terms, reference links, panels, negative prompts, or target-specific asset instructions.',
  }
  return descriptions[field] ?? field
}
