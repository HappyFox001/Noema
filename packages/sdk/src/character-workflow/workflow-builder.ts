...
function createWorkflowEditorSystemPrompt(language: CharacterWorkflowLanguage): string {
  ...
  return [
    ...
    '- image-generation-control: image count, imageStyleDomain, concise stylePrompt, shotType, aspectRatio, consistencyMode, seedMode. Use imageStyleDomain only for photoreal/anime/illustration/stylized routing; use auto when appearancePrompt and image control should decide. If the user explicitly asks for sensual/adult visual direction, merge it into stylePrompt under the current style domain. Never create a new sensual style domain, never infer sensual styling from unrelated role text, and never put imageType or composition here.',
