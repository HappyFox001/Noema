...
export function registerChatIpcHandlers(
  ...
  ipcMain.handle('chat:runCharacterWorkflow', async (_, request: ChatRunCharacterWorkflowRequest): Promise<ChatRunCharacterWorkflowResult> => {
    try {
      ...
      const tools = createDesktopCharacterAgentTools(configuredModels, options.getProxyUrl?.())
...
function createCharacterAgentModelConfigs(models: ChatIpcConfiguredModel[]): CharacterAgentModelConfig[] {
...
function createDesktopCharacterAgentTools(
  models: ChatIpcConfiguredModel[],
  proxyUrl?: string
  ...
  const tools: AgentToolDefinition[] = [
    {
      ...
      execute: async ({ callId, context }) => {
        const text = await runCharacterAgentLLMTool(context, models, proxyUrl, [
    ...
    },
    {
      ...
      execute: async ({ callId, context, input }) => {
        ...
        const imageArtifact = await maybeGenerateImageArtifact(context, models, proxyUrl, candidateId, parsed.imagePrompt)
        if (imageArtifact) {
...
async function runCharacterAgentLLMTool(
  ...
  const session = createChatSessionFromModel(toChatModelConfig({
...
function findConfiguredLLMModel(
  models: ChatIpcConfiguredModel[],
  context: CharacterAgentRunContext
): { configuredModel: ChatIpcConfiguredModel; modelName: string } {
