...
export const LOCAL_MODEL_DEFINITIONS = [
    {
        id: 'silero-vad',
        name: 'Silero VAD',
...
export async function ensureLocalModelPath(id) {
...
export async function isLocalModelAvailable(id) {
    return isValidOnnxFile(resolveLocalModelPath(id));
}
export async function downloadMissingLocalModels(resetLocalAnalyzers) {
...
async function downloadLocalModel(model) {
    const modelPath = join(resolveLocalModelsDir(), model.filename);
...
async function isValidOnnxFile(modelPath) {
    let file;
    try {
