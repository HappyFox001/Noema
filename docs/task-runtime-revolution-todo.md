...
- [x] 单个用户输入到任务执行完成的事件流可重放。
- [x] 用户打断 TTS 时只产生 speech/output 事件，不会隐式取消任务。
- [x] 任务失败和任务取消能生成不同事件链。
...
- [x] 情感层可以先回复“我来处理”，但任务是否开始由交互层/任务层决定。
...
- [x] “停一下”只停止语音，不取消任务。
- [x] “先别做这个，帮我看另一个”会暂停/保存旧任务并启动新任务。
- [x] “继续刚才那个”会恢复旧 work thread。
- [x] “刚才那个路径错了”会修改当前任务现场，而不是新开一个无上下文任务。
...
- [x] 能完成多轮代码修改任务：读文件、编辑、运行测试、修复、再验证。
...
- [x] 用户可以边聊天边让任务后台继续。
- [x] 用户可以临时换任务，再回来继续旧任务。
- [x] 系统不会因为一次语音打断误删任务现场。
...
- [x] 能跑一个真实代码指标优化任务，例如减少 TypeScript errors、修复 failing tests、提升 coverage。
...
- [x] 支持用户从 UI 手动：
  ...
  - [x] focus。
  - [x] view details。
- [x] OutputRuntime 从 work signals 接收状态，而不是从 task callback 拼状态。
...
- [x] 真实验收任务：
  ...
  - [x] 修复一批 TypeScript errors，直到 verify 通过。
  - [x] 跑一个长程优化目标，至少 3 次 iteration，有 keep 和 discard 记录。

验收：

- [x] 所有核心路径有自动化测试。
- [x] 至少一个真实长程任务从开始、打断、恢复到完成全链路通过。
- [x] 文档、事件流、持久化状态和 UI 表现一致。
...
- [x] 它能被用户语言打断，但不丢任务现场。
- [x] 它能恢复之前没做完的任务。
- [x] 它能知道上次失败在哪里，并避免重复同一失败路线。
- [x] 它能像 Codex 一样读代码、改代码、运行命令、验证、继续修。
- [x] 它能像 autoresearch 一样围绕指标长期迭代、保留有效结果、丢弃无效结果。
- [x] 它的情感层、任务层、交互层是解耦的，任何一层都能独立演进。
